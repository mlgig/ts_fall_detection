...
from scripts.ConvTran.Models.loss import l2_reg_loss
from scripts.ConvTran.Models import utils, analysis
...
class SupervisedTrainer(BaseTrainer):
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        ##### Timi's edit ###########
        self.epoch_metrics['fall_recall'] = metrics_dict['recall'][1]  # recall for the positive class (falls)
        #############################
...
def train_runner(config, model, trainer, val_evaluator, path):
    ...
    for epoch in tqdm(range(start_epoch + 1, epochs + 1), desc='Training Epoch', leave=False):
        ...
        print('aggr_metrics_train:', aggr_metrics_train)
        aggr_metrics_val, best_metrics, best_value = validate(val_evaluator, 
        tensorboard_writer, config, best_metrics, best_value, epoch)
